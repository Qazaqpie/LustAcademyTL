# TODO: Translation updated at 2022-01-25 10:33

# game/script.rpy:217
translate russian 69ff737f:

    # "Here you can keep track of all the routes, activities, etc. that are available to you. and quickly navigate to the desired one, without wasting time walking around the locations and searching. Next to the avatars of the characters are icons, the decoding of which is next."
    "Тут вы можете следить за всеми маршрутами, событиями и т.д., доступные вам. Так же быстро перемещаться между локациями, не тратя время на поиски нужного места. Рядом с аватарками персонажей расположены значки для расшифровка наведитесь на них.  "

# game/script.rpy:742
translate russian start_54c881dc:

    # NoName "Hi! My name is..."
    NoName "Привет, Мое имя ..."

# game/script.rpy:768
translate russian start_input_2_a0ecd7a8:

    # ch_Name "My name is [Name] [Surname] and I am 18 years old."
    ch_Name "Меня зовут [Name] [Surname] и мне 18 лет."

# game/script.rpy:774
translate russian start_input_2_f789cadc:

    # ch_Name "I should have been at training camp with my team right now."
    ch_Name "Я должен был быть на сборах со своей командой прямо сейчас."

# game/script.rpy:776
translate russian start_input_2_d05c9319:

    # ch_Name "But fate had something else prepared for me."
    ch_Name "Но судьба приготовила мне другое."

# game/script.rpy:780
translate russian start_input_2_8ba3057e:

    # ch_Name "Double open fracture..."
    ch_Name "Двойной открытый перелом..."

# game/script.rpy:781
translate russian start_input_2_9ca377b3:

    # ch_Name "It's a miracle I'm not crippled."
    ch_Name "Это чудо, что я не стал инвалидом."

# game/script.rpy:782
translate russian start_input_2_b118fdd0:

    # ch_Name "Still, I had to say goodbye to my soccer career..."
    ch_Name "Тем не менее, мне пришлось попрощаться с футбольной карьерой..."

# game/script.rpy:783
translate russian start_input_2_e826d333:

    # ch_Name "And to the scholarship I needed for college as well."
    ch_Name "И со стипендией, которая мне была нужна для поступления в колледж."

# game/script.rpy:787
translate russian start_input_2_9cc3f3ac:

    # ch_Name "So I had to find another way to get an education."
    ch_Name "Поэтому мне пришлось искать другой способ получить образование."

# game/script.rpy:789
translate russian start_input_2_55620e58:

    # ch_Name "Samantha suggested I try photography."
    ch_Name "Саманта предложила мне попробовать себя в качестве фотографа."

# game/script.rpy:791
translate russian start_input_2_a8a5cb72:

    # ch_Name "As it turns out, I'm a natural!"
    ch_Name "И как оказалось, я прирожденный фотограф!"

# game/script.rpy:793
translate russian start_input_2_e0b7f024:

    # ch_Name "Now I work part-time as a photographer to save money for college."
    ch_Name "Теперь я подрабатываю фотографом, чтобы накопить денег на колледж."

# game/script.rpy:798
translate russian start_input_2_0eb03f84:

    # ch_Name "I've been applying filters on photos all day..."
    ch_Name "Я весь день добавлял фильтры к фотографиям..."

# game/script.rpy:800
translate russian start_input_2_86ff8bea:

    # ch_Name "I hope the client will be happy."
    ch_Name "Я надеюсь, что клиент будет доволен."

# game/script.rpy:802
translate russian start_input_2_19d3c2fb:

    # ch_Name "A few more shoots and I'll have enough for a down payment."
    ch_Name "Еще несколько съемок, и у меня будет достаточно денег для того, что бы внести задаток за обучение."

# game/script.rpy:804
translate russian start_input_2_b9fa1026:

    # ch_Name "And I still have time to get out of this shit-hole this semester."
    ch_Name "И у меня еще есть время выбраться из этой дерьмовой дыры в этом семестре."

# game/script.rpy:818
translate russian start_input_2_b6ce45a5:

    # NoName "Knock-Knock"
    NoName "Тук-тук"

# game/script.rpy:863
translate russian start_input_2_6f8b77bd:

    # Olivia "[Name] [Surname]! You've been slacking again?"
    Olivia "[Name] [Surname]! Ты опять бездельничаешь?"

# game/script.rpy:865
translate russian start_input_2_fcb030eb:

    # Olivia "I expected as much from you."
    Olivia "Я ожидал от тебя этого."

# game/script.rpy:867
translate russian start_input_2_944e7bfb:

    # ch_Name "Please don't start that, Olivia, I've been working all day."
    ch_Name "Пожалуйста, не начинай это, Оливия, я работал весь день."

# game/script.rpy:869
translate russian start_input_2_9039396f:

    # Olivia "Start what?!"
    Olivia "Не начинать что?!"

# game/script.rpy:871
translate russian start_input_2_f6f68885:

    # Olivia "What kind of tone is that?"
    Olivia "Что это за тон такой?"

# game/script.rpy:873
translate russian start_input_2_4d4ef9ab:

    # Olivia "You've been in your room all day!"
    Olivia "Ты был весь день в своей комнате!"

# game/script.rpy:875
translate russian start_input_2_3c898584:

    # Olivia "Or do you think I'm blind?!"
    Olivia "Или ты думаешь, что я слепая?!"

# game/script.rpy:877
translate russian start_input_2_b510ad9f:

    # ch_Name "But..."
    ch_Name "Но..."

# game/script.rpy:879
translate russian start_input_2_ad8c35fe:

    # Olivia "No buts!"
    Olivia "Никаких но!"

# game/script.rpy:881
translate russian start_input_2_9a44f10c:

    # Olivia "That's what Don was talking about. You're out of control!"
    Olivia "Как Дон и говорил. Ты вышел из-под контроля!"

# game/script.rpy:883
translate russian start_input_2_3a73df72:

    # Olivia "Your ingratitude leaves us no choice."
    Olivia "Твоя неблагодарность не оставляет нам выбора."

# game/script.rpy:885
translate russian start_input_2_2a5612ed:

    # Olivia "If your mother heard that at 18 years old..."
    Olivia "Если бы твоя мать услышал, что ее 18 летний "

# game/script.rpy:887
translate russian start_input_2_362faf0b:

    # Olivia "...her son would neither study nor work!"
    Olivia "...сын ни учиться и не работает!"

# game/script.rpy:889
translate russian start_input_2_2043c397:

    # Olivia "She'd be spinning in her grave!"
    Olivia "Она бы гробу перевернулась!"

# game/script.rpy:891
translate russian start_input_2_f6a83891:

    # ch_Name "Olivia..."
    ch_Name "Оливия..."

# game/script.rpy:893
translate russian start_input_2_117ab70a:

    # ch_Name "Leave my mother alone!"
    ch_Name "Оставь мою мать в покое"

# game/script.rpy:895
translate russian start_input_2_ce40e133:

    # Olivia "Don't you raise your voice at me in my house!"
    Olivia "Не смей повышать на меня голос в моем доме!"

# game/script.rpy:897
translate russian start_input_2_532196a0:

    # Olivia "Truth hurts, doesn't it?"
    Olivia "Правда причиняет боль, не так ли?"

# game/script.rpy:899
translate russian start_input_2_f6724a46:

    # ch_Name "You know why this is the way it is!"
    ch_Name "Ты сама знаешь почему все так получилось!"

# game/script.rpy:901
translate russian start_input_2_4682ac22:

    # ch_Name "I'd be at university right now if it wasn't for my leg..."
    ch_Name "Я бы сейчас учился в университете, если бы не моя нога..."

# game/script.rpy:903
translate russian start_input_2_6eb4f2dc:

    # Olivia "You should have thought of that before!"
    Olivia "Тебе следовало подумать об этом раньше!"

# game/script.rpy:905
translate russian start_input_2_245150b8:

    # Olivia "Sport is always a risk."
    Olivia "Спорт это всегда риск."

# game/script.rpy:907
translate russian start_input_2_5783fa11:

    # Olivia "You should've done less loafing and studied more. Like Ashley!"
    Olivia "Тебе следовало меньше бездельничать и больше учиться. Как Эшли!"

# game/script.rpy:909
translate russian start_input_2_aedee487:

    # Olivia "And now what?"
    Olivia "И что теперь?"

# game/script.rpy:911
translate russian start_input_2_036404a6:

    # Olivia "How do you plan to get an education?"
    Olivia "Как ты планируешь получать высшее образование?"

# game/script.rpy:913
translate russian start_input_2_b1e52819:

    # ch_Name "I'm working on it."
    ch_Name "Я работаю над этим."

# game/script.rpy:915
translate russian start_input_2_2aa9b9d5:

    # Olivia "He's working..."
    Olivia "Работает он.."

# game/script.rpy:917
translate russian start_input_2_b33dc905:

    # Olivia "I see how hard you’re \"working\"!"
    Olivia "Я вижу, как ты усредно \"работаешь\"!"

# game/script.rpy:919
translate russian start_input_2_691c3787:

    # Olivia "[Name], our hospitality is not unlimited."
    Olivia "[Name],пойми наше гостеприимтсво не вечно. "

# game/script.rpy:921
translate russian start_input_2_2089379b:

    # ch_Name "{i}(That's for sure...){/i}"
    ch_Name "{i}(Это уж точно...){/i}"

# game/script.rpy:923
translate russian start_input_2_2869137c:

    # Olivia "We didn't want this, but you leave us no choice."
    Olivia "Мы не хотели этого, но ты не оставляешь нам выбора."

# game/script.rpy:925
translate russian start_input_2_42f41d8b:

    # Olivia "If you want to throw your life away..."
    Olivia "Если ты попусту хочешь растратить свою жизнь..."

# game/script.rpy:927
translate russian start_input_2_b962a7e0:

    # Olivia "We won't let you do it with your feet dangling off Don's neck!"
    Olivia "Мы не позволим тебе сидеть на шее Дона"

# game/script.rpy:929
translate russian start_input_2_f6f5841f:

    # Olivia "You're a grown man."
    Olivia "Ты взрослый мужчина."

# game/script.rpy:931
translate russian start_input_2_bca2c05c:

    # Olivia "You can take care of yourself."
    Olivia "Ты можешь сам о себе позаботиться."

# game/script.rpy:933
translate russian start_input_2_f4cb3c7a:

    # ch_Name "What do you mean? Are you kicking me out?"
    ch_Name "Что ты имеешь в виду? Ты что, выгонишь меня?"

# game/script.rpy:935
translate russian start_input_2_90f3a346:

    # ch_Name "But, Olivia, come on... Where am I supposed to go?"
    ch_Name "Но, Оливия, перестань... Куда я пойду?"

# game/script.rpy:937
translate russian start_input_2_2661214d:

    # Olivia "What do you take us for? Savages?"
    Olivia "За кого ты нас принимаешь? За дикарей?"

# game/script.rpy:939
translate russian start_input_2_3c004877:

    # Olivia "You're my best friend's son, God rest her soul."
    Olivia "Ты сын моей лучшей подруги, упокой Господь ее душу."

# game/script.rpy:941
translate russian start_input_2_668cf83e:

    # Olivia "And I talked Don out of kicking you out."
    Olivia "И я уговорил Дона не выгонять тебя."

# game/script.rpy:943
translate russian start_input_2_cbbc98eb:

    # ch_Name "Thank you, I knew..."
    ch_Name "Спасибо, я знал, что..."

# game/script.rpy:945
translate russian start_input_2_53623f8c:

    # Olivia "But!"
    Olivia "Но!"

# game/script.rpy:947
translate russian start_input_2_7e4ac717:

    # ch_Name "{i}(Of course there’s a \"but\"!){/i}"
    ch_Name "{i}(Ну конечно всегда есть\"Но\"! ){/i}"

# game/script.rpy:949
translate russian start_input_2_09453d86:

    # Olivia "We're done supporting you."
    Olivia "Мы устали тебя опекать"

# game/script.rpy:951
translate russian start_input_2_a2fcf8a5:

    # ch_Name "What?!"
    ch_Name "Что?!"

# game/script.rpy:953
translate russian start_input_2_eb708d4a:

    # Olivia "From now on, if you want to live here..."
    Olivia "С этого момента, если ты хочешь жить здесь..."

# game/script.rpy:955
translate russian start_input_2_1394e177:

    # Olivia "Pay your rent as a lodger."
    Olivia "Плати арендную плату, как квартирант."

# game/script.rpy:957
translate russian start_input_2_306af026:

    # ch_Name "What?! What?!"
    ch_Name "Что?! Что?!"

# game/script.rpy:959
translate russian start_input_2_97dd3450:

    # Olivia "I expect payment by the fifth of every month."
    Olivia "Я ожидаю оплаты к пятому числу каждого месяца."

# game/script.rpy:961
translate russian start_input_2_8ac87752:

    # ch_Name "You're kidding, right?"
    ch_Name "Ты шутишь, да?"

# game/script.rpy:963
translate russian start_input_2_833ce195:

    # Olivia "You don't have to pay for August, but..."
    Olivia "Тебе не нужно платить за август, но..."

# game/script.rpy:965
translate russian start_input_2_db8f91ff:

    # ch_Name "Olivia, hey. Tell me you're kidding..."
    ch_Name "Оливия. Эй . Скажи мне, что ты шутишь..."

# game/script.rpy:967
translate russian start_input_2_3da42f20:

    # Olivia "I need your first and last month's payment in September."
    Olivia "Мне нужна твоя оплата за первый и последний месяц в сентябре."

# game/script.rpy:969
translate russian start_input_2_a680031d:

    # Olivia "Plus a security deposit in case of property damage."
    Olivia "Плюс страховой залог на случай порчи имущества."

# game/script.rpy:971
translate russian start_input_2_b863e1f9:

    # ch_Name "What... what kind of nonsense is that?!"
    ch_Name "Что... что это за чушь такая?!"

# game/script.rpy:973
translate russian start_input_2_aa82ecc0:

    # ch_Name "How am I supposed to save for school while paying rent? It's a vicious circle!"
    ch_Name "Как я должен откладывать на учебу, оплачивая аренду? Это порочный круг!"

# game/script.rpy:975
translate russian start_input_2_a889e973:

    # Olivia "Young man!"
    Olivia "Молодой человек!"

# game/script.rpy:977
translate russian start_input_2_419bb65f:

    # Olivia "You're under my roof."
    Olivia "Ты под моей крышей."

# game/script.rpy:979
translate russian start_input_2_b59257ff:

    # Olivia "You follow my rules."
    Olivia "Значит ты следуешь моим правилам."

# game/script.rpy:981
translate russian start_input_2_0c3ee8a6:

    # Olivia "You don't like our offer?"
    Olivia "Тебе не нравится наше предложение?"

# game/script.rpy:983
translate russian start_input_2_143d02b4:

    # Olivia "Go into the living room and say it to Don's face."
    Olivia "Иди в гостиную и скажи это Дону в лицо."

# game/script.rpy:985
translate russian start_input_2_82ed18d6:

    # Olivia "Or do you only have the courage to bicker with me?"
    Olivia "Или у тебя хватает смелости на то, чтобы спорить только со мной?"

# game/script.rpy:987
translate russian start_input_2_b8f6f104:

    # ch_Name "{i}(Damn it, I knew I shouldn't have fallen for her provocations...){/i}"
    ch_Name "{i}(Черт, я знал, что не должен был поддаваться на ее провокации...){/i}"

# game/script.rpy:989
translate russian start_input_2_931280c0:

    # ch_Name "All right. I'll talk to Don."
    ch_Name "Окей. Я поговорю с Доном"

# game/script.rpy:991
translate russian start_input_2_512eba66:

    # ch_Name "Maybe he won't be so deaf to reason."
    ch_Name "Может хотя бы он меня послушает"

# game/script.rpy:993
translate russian start_input_2_fe2acc96:

    # Olivia "Good luck with that."
    Olivia "Удачи тебе в этом."

# game/script.rpy:995
translate russian start_input_2_86f4b2fc:

    # Olivia "After all, it was his idea."
    Olivia "В конце концов, это была его идея."

translate russian strings:

    # game/script.rpy:61
    old "Nice"
    new "Приятный"

    # game/script.rpy:63
    old "Flirty"
    new "Кокетлевый"

    # game/script.rpy:65
    old "Troll"
    new "Подколоть"

    # game/script.rpy:67
    old "Angry"
    new "Злой"

    # game/script.rpy:69
    old "Flirt"
    new "Флирт"

    # game/script.rpy:71
    old "Support"
    new "Поддержка"

    # game/script.rpy:73
    old "Joke"
    new "Шутка"

    # game/script.rpy:75
    old "Funny"
    new "Смешно"

    # game/script.rpy:77
    old "Sad"
    new "Грустно"

    # game/script.rpy:79
    old "Pessimistic"
    new "Пессимистично"

    # game/script.rpy:81
    old "Fun"
    new "Весело"

    # game/script.rpy:84
    old "Lesser Heal"
    new "Малое исцеление"

    # game/script.rpy:86
    old "Stoneskin"
    new "Каменная кожа"

    # game/script.rpy:88
    old "Inspiration"
    new "Воодушевление"

    # game/script.rpy:90
    old "Magic Reflection"
    new "Магичесткое отражение"

    # game/script.rpy:93
    old "CALENDAR"
    new "КАЛЕНДАРЬ"

    # game/script.rpy:95
    old "CHARACTER INFO"
    new "ИНФО О ПЕРСОНАЖАХ"

    # game/script.rpy:97
    old "MY PROFILE"
    new "МОЙ ПРОФИЛЬ"

    # game/script.rpy:100
    old "Makeup bag"
    new "Косметичка"

    # game/script.rpy:102
    old "Money"
    new "Деньги"

    # game/script.rpy:104
    old "Documents"
    new "Документы"

    # game/script.rpy:106
    old "Condoms"
    new "Презервативы"

    # game/script.rpy:108
    old "Book"
    new "Книга"

    # game/script.rpy:110
    old "Corridor"
    new "Коридор"

    # game/script.rpy:112
    old "Living Room"
    new "Гостинная"

    # game/script.rpy:114
    old "Girls Room 1"
    new "Комната девочек 1"

    # game/script.rpy:116
    old "Girls Room 2"
    new "Комната девочек 2"

    # game/script.rpy:118
    old "My Room"
    new "Моя комната"

    # game/script.rpy:120
    old "Sleep"
    new "Сон"

    # game/script.rpy:122
    old "Inner Garden"
    new "Внутренний сад"

    # game/script.rpy:124
    old "Entrance"
    new "Фойе"

    # game/script.rpy:126
    old "Left Wing"
    new "Левое крыло"

    # game/script.rpy:128
    old "Right Wing"
    new "Правое крыло"

    # game/script.rpy:130
    old "Main Hall"
    new "Главный зал"

    # game/script.rpy:132
    old "Library"
    new "Библиотека"

    # game/script.rpy:134
    old "Classroom 1"
    new "Кабинет 1"

    # game/script.rpy:136
    old "Cafe"
    new "Кафе"

    # game/script.rpy:138
    old "Classroom 2"
    new "Кабинет 2"

    # game/script.rpy:140
    old "Bathroom"
    new "Уборная"

    # game/script.rpy:142
    old "Elijah's Room"
    new "Комната Элайджи"

    # game/script.rpy:144
    old "Outside"
    new "Наружу"

    # game/script.rpy:146
    old "Upstairs"
    new "Наверх"

    # game/script.rpy:148
    old "Sorting book"
    new "Распределяющая книга"

    # game/script.rpy:150
    old "Ashley's Bedroom"
    new "Спальня Эшли"

    # game/script.rpy:152
    old "Samantha's Bedroom"
    new "Спальня Саманты"

    # game/script.rpy:154
    old "Downstairs"
    new "Вниз"

    # game/script.rpy:156
    old "My Bedroom"
    new "Моя спальня"

    # game/script.rpy:158
    old "Olivia's Bedroom"
    new "Спальня Оливии"

    # game/script.rpy:160
    old "Cupboard"
    new "Шкаф"

    # game/script.rpy:162
    old "Olivia’s Purse"
    new "Сумочка Оливии"

    # game/script.rpy:164
    old "Photo"
    new "Фото"

    # game/script.rpy:166
    old "Painting"
    new "Картина"

    # game/script.rpy:168
    old "Bath Ducks"
    new "Резиновые утки"

    # game/script.rpy:170
    old "Toothbrush"
    new "Зубная паста"

    # game/script.rpy:172
    old "Viewpoint"
    new "Точка обзора"

    # game/script.rpy:174
    old "Main Street"
    new "Главная улица"

    # game/script.rpy:176
    old "Coffee Hut"
    new ""

    # game/script.rpy:178
    old "Alley"
    new "Аллея"

    # game/script.rpy:180
    old "Club"
    new "Клуб"

    # game/script.rpy:182
    old "Hotel"
    new "Отель"

    # game/script.rpy:184
    old "Shop"
    new "Магазин"

    # game/script.rpy:186
    old "Spa"
    new "Спа"

    # game/script.rpy:188
    old "Exit"
    new "Выход"

    # game/script.rpy:190
    old "Room"
    new "Комната"

    # game/script.rpy:192
    old "Campus"
    new "Кампус"

    # game/script.rpy:194
    old "Academy"
    new "Академия"

    # game/script.rpy:196
    old "Dale"
    new "Дейл"

    # game/script.rpy:206
    old "All characters are 18 age or older even if specified otherwise."
    new "Все персонажи достигли 18 лет или даже старше, если указать."

    # game/script.rpy:207
    old "This game contains violence, obscene scenes (naked girls and sex scenes), "
    new "Эта игра содержит насилие, непристойные сцены (голые девушки и сексуальные сцены),"

    # game/script.rpy:208
    old "rude humor, scenes that depict smoking or infrequent use of profanity."
    new "грубый юмор, сцены, изображающие курение или использование ненормативной лексики."

    # game/script.rpy:330
    old "Warning!"
    new "Предупреждение!"

    # game/script.rpy:331
    old "In order to maximize the enjoyment of the game we have added music and ambient sounds, including those of an erotic nature."
    new "Чтобы максимально насладиться игрой, мы добавили музыку и окружающие звуки, в том числе эротического характера."

    # game/script.rpy:332
    old "You can adjust the volume and turn off the music in the settings."
    new "Вы можете отрегулировать громкость и отключить музыку в настройках."

    # game/script.rpy:333
    old "We hope this did not cause you any trouble."
    new "Мы надеемся, что это не доставило вам никаких хлопот."

    # game/script.rpy:334
    old "Enjoy the game!"
    new "Наслаждайтесь игрой!"

    # game/script.rpy:340
    old "Note that our game features LGBTQ+ scenes. If you're not interested in stuff like that at all, you can turn this option"
    new "Обратите внимание, что в нашей игре есть сцены ЛГБТК +. Если вас не интересуют подобные вещи, вы можете выключить эту опцию"

    # game/script.rpy:340
    old "off right now (you can always turn it on later, in the settings). Leave this option enabled if you're only"
    new "сейчас.(Вы всегда можете включить его позже, в настройках). Оставьте эту опцию включенной, если вы только"

    # game/script.rpy:340
    old "interested in some of the possible content. Don't worry! We'll ask you beforehand each"
    new "заинтересованы таким возможным контентом. Не волнуйся! Мы будем спрашивать заранее каждый раз"

    # game/script.rpy:340
    old "time there's a chance to encounter such interactions."
    new "Когда есть шанс столкнуться с такими взаимодействиями."

    # game/script.rpy:340
    old "Do you want to enable LGBTQ+ content?"
    new "Вы хотите включить контент ЛГБТК +?"

    # game/script.rpy:351
    old "Enable"
    new "Включить"

    # game/script.rpy:352
    old "Disable"
    new "Выключить"

    # game/script.rpy:356
    old "On Sunday evening, all students return to Cordale. So you did."
    new "В воскресенье вечером все студенты возвращаются в Кордейл. Так ты и сделал."

    # game/script.rpy:728
    old "{b}Olivia{/b} said I have to talk to Don about this rent thing. \nHe should be in the living room."
    new "{b} Оливия {/b} сказала, что я должен поговорить с Доном об этой арендной плате. \nОн должен быть в гостиной."

    # game/script.rpy:748
    old "Enter your name"
    new "Введите свое имя"

    # game/script.rpy:748
    old "James"
    new "Джеймс"

    # game/script.rpy:759
    old "Enter your surname"
    new "Введите свою фамилию"

    # game/script.rpy:759
    old "Поггер"
    new ""

    # game/script.rpy:807
    old "Continue"
    new "Продолжить"

    # game/script.rpy:807
    old "Skip prologue"
    new "Пропустить пролог"

